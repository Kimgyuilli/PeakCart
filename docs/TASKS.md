...
| ③ | Spring Cloud Gateway (PR1 ✅ [#73](https://github.com/Kimgyuilli/PeakCart/pull/73) RS256/JWKS dual-validation — `JwtKeyProperties`/`PemKeyLoader`/`RsaPublicKeyRegistry`·verifier alg allow-list(kid 선택·HS512 fallback bounded off)·User RS256 서명(kid)+JWKS `/.well-known/jwks.json`·개인키 산출물 비포함(testFixtures/파일마운트)·latency 측정, ADR-0013 D1/D2) · PR2 Reuse Detection / PR3 Gateway 모듈+header-trust / PR4 관측성 S9 대기 | A4 | 보안 묶음 | 🔄 |
