...
3. **구현 ① 진행 중**: PR1 ✅ · PR2a(Notification peel) ✅ · PR2b(User peel) ✅ · 사가 클러스터 strangler-1(재고 예약/복구) ✅ [#56] · strangler-2(단가 캐시 CQRS) ✅ [#57] · strangler-3(2-phase 확정/보상 + 결제 게이트) ✅ [#58] · strangler-4(`verifyProductExists` 로컬 캐시화·ProductPort 동기 seam 제거) ✅ [#61] → Order↔Product 동기 결합 0 · **Product peel ✅ [#62] product-service 모듈 분리(첫 *발행* 서비스 — outbox/idempotency/ShedLock 복제·공유 DB poller 소유권 분리·root 테스트 디커플). DB 물리분리는 범위 외(② 일괄 이연, 공유 root DB·Flyway runtime disabled 유지)** · **strangler-5(Order↔Payment 동기 결합 제거) ✅ [#63] `OrderPort` seam 제거→payment-로컬 게이트+`payment.requested` 이벤트, ADR-0012 §D4 refine** → Order↔Payment 동기 결합 0 — **다음 = Order+Payment peel(root app 소멸, strangler-5 계획서 입력) → PR3(Dockerfile/CI/k8s)**.
