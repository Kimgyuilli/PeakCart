...
/**
 * product-service 의 DLQ quarantine 경로 (계획 ④-c-2a P7).
 *
...
public class DeadLetterQuarantineConsumer {
    ...
    private static final PeekcartService SELF = PeekcartService.PRODUCT;

    private final DeadLetterRecorder recorder;

    @KafkaListener(
            id = DeadLetterContainerGuard.LISTENER_ID_PREFIX + "product-quarantine",
            topics = {
                    "product.updated.dlq",
                    "stock.reservation.result.dlq",
                    "stock.compensation.requested.dlq"
            },
            groupId = DlqTopology.PRODUCT_DLQ_QUARANTINE_GROUP,
