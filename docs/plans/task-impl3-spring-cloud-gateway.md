...
| `TokenBlacklistLookupPort`/`RedisTokenBlacklistLookupAdapter` — common-auth, 5서비스 | 서비스별 blacklist 읽기 | **이동**: blacklist/deny 확인 = Gateway 소유. 리소스 서비스는 미확인(헤더 신뢰). read adapter 는 Gateway 전용화 |
...
- [ ] **P6.** `refresh_tokens` 마이그레이션(user-service `V{n}__`): `family_id`·`token_hash`·`status`(ACTIVE/ROTATED/REVOKED)·`rotated_at`·`grace_until`·`replaced_by_token_id` 추가. 삭제 기반 → 상태전이. 조회 인덱스(`token_hash`·`family_id`). **기존 데이터 처분(명시)**: 그린필드(보존 prod 이력 없음, impl② 선례)라 **기존 `refresh_tokens` row 전량 무효화(재로그인 요구)** 채택. 기존 `token`(평문 UUID)을 해시해 `token_hash`/`family_id=신규`/`status=ACTIVE` 로 backfill 하는 경로는 *기술적으로 가능하나* 보존 요구가 없어 채택하지 않고 전량 만료로 단순화. access token 은 짧은 TTL 로 자연 소멸.
- [ ] **P7.** `RefreshToken` 엔티티 + repository 확장: 상태전이 rotation, family 단위 조회/무효화, token_hash 조회(평문 미저장).
- [ ] **P8.** `AuthService.refresh` 재작성: **grace**(정상 동시요청 = `grace_until` 내 1회성 허용) vs **reuse**(grace 초과 + ROTATED/REVOKED 재제시, 또는 이미 revoked family 재제시) → **family 전체 무효화**.
- [ ] **P9.** family/session deny → Redis 기록(Gateway blacklist source): reuse 감지 시 family(또는 session) deny 키 write(User 소유). access token 에 `family_id`(또는 `session_id`) claim 추가 — `JwtTokenSigner` + `TokenClaims`.
- [ ] **P10.** PR2 테스트: rotation 상태전이, grace 1회 허용/2회 차단, reuse 감지→family revoke→Redis deny write, family_id claim 왕복, revoked family 재제시 거부. **+ 마이그레이션 회귀**: Flyway 마이그레이션 적용 후 기존 row 전량 무효화 확인 + 기존(만료된) refresh token 재제시 시 `AuthService.refresh` 가 안전 거부(NPE/오통과 없음).
...
- **PR2**: `user-service` `refresh_tokens` 마이그레이션·`RefreshToken.java`·`RefreshTokenRepository(Impl/Jpa)`·`AuthService.java`·`TokenBlacklistPort`/deny 확장·`TokenClaims`(family_id).
...
- **PR2**: `./gradlew :user-service:test` — grace 1회/2회 차단, reuse→family revoke→Redis deny write 확증(Testcontainers Redis).
...
- [ ] reuse 재제시 → family 전체 무효화 + Redis family deny → 이미 발급된 access token 즉시 차단 확증. (PR2/PR3)
