...
/**
 * Notification 서비스 보안 통합 회귀 — Gateway 서명 내부 토큰 전환(ADR-0017 · PR3d).
 * 평문 {@code X-User-*} 신뢰를 폐기하고 {@code X-Internal-Auth} 서명만 인증 근거로 삼는다.
...
@DisplayName("Notification 보안 통합 테스트 (Gateway 서명 내부 토큰)")
class NotificationSecurityIntegrationTest {
    ...
    void validSignedToken_authenticated() {
        ResponseEntity<String> res = restTemplate.exchange(
                "/api/v1/notifications", HttpMethod.GET, signed(1L, "USER", "fam-1"), String.class);
        assertThat(res.getStatusCode()).isEqualTo(HttpStatus.OK);
    }

    @Test
    @DisplayName("위조 서명 토큰은 401 (미승인 키로 서명 — 서명 검증이 실제로 동작하는지 확인)")
    void forgedToken_rejected() {
        HttpHeaders headers = new HttpHeaders();
        headers.set(InternalTokenContract.HEADER, InternalTokenFixtures.mintForged(999L, "ADMIN"));
        ResponseEntity<String> res = restTemplate.exchange(
    ...
    @Test
    @DisplayName("평문 X-User-* 직접 주입은 무시된다 → 보호 경로 401 (스푸핑 표면 0)")
    void plaintextHeaders_ignored() {
        ...
        assertThat(res.getStatusCode())
                .as("평문 헤더로 인증되면 NetworkPolicy 를 뚫은 직접 요청이 그대로 신원을 위조한다")
    ...
    void directBearerToken_rejected() {
        ...
        ResponseEntity<String> res = restTemplate.exchange(
                "/api/v1/notifications", HttpMethod.GET, new HttpEntity<>(headers), String.class);
        assertThat(res.getStatusCode())
                .as("서비스가 사용자 토큰을 다시 검증하면 내부 서명 없이 Gateway 를 우회할 수 있다")
                .isEqualTo(HttpStatus.UNAUTHORIZED);
    }

    @Test
    @DisplayName("SecurityFilterChain 은 정확히 1개, 내부 토큰 필터는 1회만 등록된다 (configurer 동등성)")
    void singleChain_singleInternalTokenFilter() {
        assertThat(securityFilterChains).hasSize(1);
        SecurityFilterChain chain = securityFilterChains.values().iterator().next();
        long filterCount = chain.getFilters().stream()
    ...
    private HttpEntity<Void> signed(long userId, String role, String familyId) {
        HttpHeaders headers = new HttpHeaders();
        headers.set(InternalTokenContract.HEADER, InternalTokenFixtures.mint(userId, role, familyId));
