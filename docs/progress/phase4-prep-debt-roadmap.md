...
| L-006 | Resilience | Redis 조회 캐시 fallback(`CacheErrorHandler`) — Redis 가 서비스간 공유 인프라화 시. L-005(작업4) 선결 | ✅ 구현 ⑤ (`ResilientCacheErrorHandler` — get/put fail-open, evict/clear 는 WARN+`cache_fallback_total`) |
...
| **D-019** | OutboxKafka E2E `orderCancelled` flaky — D-013 producer 타임아웃 타이트화 여파(단발 poll 테스트 미수정). 타이밍 flake 확정, 프로덕션 회귀 아님 | 1 (마무리분) | 완료 |
...
1. ~~신규 버킷 1(D-015/D-017/D-018) 처리. D-015 는 배포 계약 불일치라 최우선.~~ ✅ 완료 (도식검토 부채 완결, PR #40/#41/#42). ~~D-019 flaky 봉합~~ ✅ 완료 (PR #43, 버킷 1 마무리분).
