...
- **Helm 버전 호환성**: 로컬에 설치된 `helm v4.1.3` 은 계획서의 `≥3.12` 최소 조건을 만족하지만 메이저 업. `k8s/monitoring/gke/install.sh` (helm 3 기준 작성) 는 세션 B Step 2 monitoring 설치 시 dry-run 경로 없이 바로 실행되므로, **세션 B 진입 직후 monitoring 설치 전 `helm template` 한 번 돌려 경고/에러 유무 확인** 필요. 호환 이슈 발견 시 해당 세션에서 튜닝 금지 → cleanup 후 로컬 helm 3 설치 또는 install.sh 수정
