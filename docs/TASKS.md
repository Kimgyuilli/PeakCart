...
| **리뷰 개선 P1-E**: Error Rate PromQL NaN 가드 | 🔄 구현 완료, 런타임 검증 pending | `api-jvm-dashboard.json` / `dashboards-configmap.yml` 동기: `(A / (B > 0) * 100) or vector(0)` (idle 구간 0% 강제). `grafana-alerts.yml`: `($B > 0) && (($A / $B) * 100 > 5)` (Grafana math `&&` 문법). **신호 손실 보완 구현**: Service Up stat 패널 + `peekcart-target-down` (up==0) + `peekcart-scrape-absent` (series 부재) 알림 2건 분리 추가. **런타임 검증 pending** — monitoring 스택 기동 시점에 Grafana Preview/패널 렌더링 + 알림 firing 실증 필요 (✅ 는 검증 완료 후 전환) |
