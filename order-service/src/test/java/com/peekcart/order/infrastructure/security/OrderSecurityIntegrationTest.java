...
/**
 * order-service 보안 통합 회귀 — Gateway 서명 내부 토큰(ADR-0017 · PR3d).
 *
...
@DisplayName("order-service 보안 통합 테스트 (Gateway 서명 내부 토큰)")
class OrderSecurityIntegrationTest {
    ...
    @Test
    @DisplayName("유효 서명 내부 토큰 → 인증 통과 (OK — 401/403/5xx 배제)")
    void validSignedToken_authenticated() {
        // 빈 주문 목록 — 인증 통과 시 200 이 결정적이다.
