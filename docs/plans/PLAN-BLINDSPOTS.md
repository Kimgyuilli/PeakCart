...
- **Check (인프라→공유리소스 이름 간선)**: 단일 공유 리소스(Secret/ConfigMap)를 per-service 로 **쪼갤** 때, 그 리소스를 **인프라 매니페스트가 이름으로 참조**(예: MySQL Deployment 의 `secretKeyRef.name: peekcart-secret`)하는 간선을 놓치기 쉽다 — 스윕이 "앱" 쪽(image/deployment/lint)에만 쏠리면 인프라→secret 간선이 빠져 rename/삭제 후 `CreateContainerConfigError` 로 클러스터가 부팅 못 한다. 분해 대상 리소스의 **`name:` 을 전 매니페스트에서 grep**(`grep -rn "name: <resource>" k8s/`)해 인프라 참조까지 처분(분리된 인프라 전용 리소스 신설 등)하라.
- **출처**: task-impl-product-peel — FQCN grep 으로 src/main 0건·src/test 5건만 처분했으나, 빌드에서 `ObservabilityMetricsIntegrationTest`(`/api/v1/products`·`"products"` 캐시 프록시)·`ShedLockIntegrationTest`(`outboxPollingJob` 락 이름)·outbox probe aggregateType·product 통합테스트 `flyway.enabled` 4부류가 런타임 실패. peel 대상의 string-level 식별자 미스윕. · PR3b(k8s secret 분해) — 단일 `peekcart-secret` 을 5 per-service secret 으로 쪼개며 **MySQL Deployment 의 `secretKeyRef.name: peekcart-secret`** 간선을 스윕이 누락 → 렌더는 통과하나 배포 시 MySQL CreateContainerConfigError(work GW-2 P0). `infra/mysql/secret.yml` 분리로 봉합.
...
- **Check (역방향 — 함정)**: B6 의 처분(`@ConditionalOnProperty` 로 사용 모듈만 활성)이 적용된 :common 빈을 **실제로 생성자 주입(required)하는 서비스가 여럿**이면, 그 서비스들에 **conditioning 프로퍼티/secret 을 per-service 로 공급하지 않는 한 부팅 실패**(`UnsatisfiedDependency` — 빈 부재). 즉 "미사용 모듈 비활성" 과 "사용 모듈에서 secret 누락 시 fail" 은 동전의 양면. peel/배포 분할 시 **그 빈을 생성자 주입하는 서비스 전부를 grep**(`grep -rln SlackPort --include=*.java <each-service>`)으로 뽑아, 각 서비스에 (a) secret 주입 또는 (b) `@ConditionalOnMissingBean` no-op fallback 빈을 계획에 명시. fallback 없으면 secret 을 알림 비핵심 서비스까지 강제 배포해야 함.
- **Check (함정² — presence-based 조건 + 기본값)**: `@ConditionalOnProperty(name="x")` 는 **havingValue 없으면 "값 존재(≠false)" 만으로 매치** = presence-based. 이때 base yml 이 `x: ${ENV:placeholder}` **기본값**을 두면 env 누락에도 property 가 항상 존재 → **조건이 항상 true** → "secret 누락 시 fail-fast" 도, "no-op fallback 선택" 도 **둘 다 깨진다**(real bean 이 placeholder 로 무조건 등록). no-op/fail-fast 게이팅을 설계할 땐 **base yml 의 placeholder 기본값 제거**(누락=property 부재)가 선결. real(`@ConditionalOnProperty(x)`) ↔ no-op(`@ConditionalOnMissingBean`+`@ConditionalOnProperty(noop.enabled)`) 이 **동시 true 불가**하도록 명시 property 로 상호배타화(@ConditionalOnMissingBean 단독 의존 금지). base 기본값 제거는 local/test 프로파일도 깨므로 동반 처분 필요.
- **출처**: PR2b — user-service 가 `SlackNotificationClient`(@Value 필수)로 `PlaceholderResolution` 부팅 실패 → `@ConditionalOnProperty` · `KafkaAutoConfiguration` exclude · `spring-boot-starter-validation` 누락으로 @Valid 미작동 500. PR3b(k8s per-service secret) — `SlackNotificationClient` `@ConditionalOnProperty(slack.webhook.url)` 를 notification/product/order/payment **4서비스가 생성자 주입** → webhook secret 미공급 시 4서비스 부팅 실패(plan GP-2 #3). no-op fallback vs 전 서비스 webhook 주입이 처분.
...
- **work: diff 캡처 전 untracked 신규 파일 intent-to-add** — `hpx_diff_capture`(=`git diff <base>`)는 **untracked 신규 파일을 보여주지 않는다** → 새 파일이 통째로 Codex diff 리뷰에서 누락된다(신규 클래스·스크립트·매니페스트 전부). `/work` Step 6 diff 캡처 직전에 `git add -N $(git ls-files --others --exclude-standard)` 로 intent-to-add 해 신규 파일을 diff 에 포함시켜야 한다. (출처: 구현 ③ PR3c — HeaderAuthenticationFilter·networkpolicy.yml·2개 lint/smoke 스크립트가 첫 캡처에서 전부 빠졌고 수동으로 add -N)
