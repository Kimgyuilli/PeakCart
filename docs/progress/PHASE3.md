...
- `MdcRecordInterceptor` — `RecordInterceptor<String,String>` 구현체. `intercept()` 에서 MDC 주입, `success`/`failure`/`afterRecord` 3중 hook 으로 정리
...
- **3중 정리 hook**: `success`/`failure`/`afterRecord` 모두에서 `MDC.remove` — Spring Kafka 버전·예외 경로별 호출 차이를 흡수하는 방어적 설계
