...
/**
 * notification-service 의 DLQ 원장 적재 통합 테스트 (계획 ④-c-2a §6 P6·P7).
 *
...
@DisplayName("notification-service DLQ 원장 적재 통합 테스트")
class DeadLetterLedgerIntegrationTest extends AbstractIntegrationTest {
    ...
    private static final String OWNED_GROUP = "notification-svc-payment-completed-group";
    ...
    @Test
    @DisplayName("발행 토픽이 없으므로 quarantine listener 도 없다 — group 부재분은 발행 서비스 소관")
    void hasNoQuarantineListener() {
        sendWithoutGroup(DLQ_TOPIC, ORIGIN_TOPIC, 0, 55L);
        // 대조군: 자기 group 은 적재된다
        send(DLQ_TOPIC, ORIGIN_TOPIC, 0, 56L, OWNED_GROUP);

        await().atMost(Duration.ofSeconds(30)).untilAsserted(() ->
                assertThat(repository.findAll()).hasSize(1));

        assertThat(repository.findAll().get(0).getOriginOffset()).isEqualTo(56L);
    }
